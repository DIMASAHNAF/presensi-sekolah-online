@props([
    'size' => '8px',
    'id' => 'sky-toggle-' . uniqid(),
    'class' => '',
])

<div class="sky-toggle-wrapper {{ $class }}" style="--toggle-size: {{ $size }}; display: inline-flex; align-items: center; flex-shrink: 0;">
  <label class="theme-switch" for="{{ $id }}" title="Ganti Mode Terang / Gelap">
    <input type="checkbox" id="{{ $id }}" class="theme-switch__checkbox" onchange="window.toggleAppTheme(this.checked)" />
...
<script>
  if (typeof window.toggleAppTheme === 'undefined') {
    window.toggleAppTheme = function(isDark) {
      if (isDark) {
    ...
    if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', syncSkyToggleState);
    } else {
      syncSkyToggleState();
    }
  }
