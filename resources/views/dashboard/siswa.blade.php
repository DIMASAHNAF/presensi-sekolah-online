...
<div x-data="{ openAdd: false, openEdit: false, editData: {} }">
    ...
    <div x-show="openAdd" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 backdrop-blur-xs px-4">
        <div @click.away="openAdd = false" class="bg-white rounded-xl shadow-xl border border-slate-200 w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
            <h3 class="font-heading font-bold text-base mb-4 text-slate-900">Tambah Data Siswa</h3>
            <form action="{{ route('dashboard.siswa.store') }}" method="POST">
                ...
                <div class="mb-4">
                    <label class="block text-xs font-semibold uppercase tracking-wider text-slate-600 mb-1.5">NISN</label>
                    <input type="text" name="nisn" class="w-full bg-white border border-slate-300 rounded-lg px-3.5 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-blue-600" required>
                </div>
                <div class="mb-4">
                    <label class="block text-xs font-semibold uppercase tracking-wider text-slate-600 mb-1.5">Kelas</label>
                    <select name="kelas_id" class="w-full bg-white border border-slate-300 rounded-lg px-3.5 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-600" required>
                ...
                <div class="flex gap-2 justify-end pt-2 border-t border-slate-100">
                    ...
                    <button type="submit" class="btn-primary py-2 text-xs">Simpan Data Siswa</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div x-show="openEdit" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 backdrop-blur-xs px-4">
        <div @click.away="openEdit = false" class="bg-white rounded-xl shadow-xl border border-slate-200 w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
            <h3 class="font-heading font-bold text-base mb-4 text-slate-900">Edit Data Siswa</h3>
            <form :action="'{{ route('dashboard.siswa') }}/' + editData.id" method="POST">
                ...
                <div class="mb-4">
                    <label class="block text-xs font-semibold uppercase tracking-wider text-slate-600 mb-1.5">NISN</label>
                    <input type="text" name="nisn" x-model="editData.nisn" class="w-full bg-white border border-slate-300 rounded-lg px-3.5 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-blue-600" required>
                </div>
                <div class="mb-4">
                    <label class="block text-xs font-semibold uppercase tracking-wider text-slate-600 mb-1.5">Kelas</label>
                    <select name="kelas_id" x-model="editData.kelas_id" class="w-full bg-white border border-slate-300 rounded-lg px-3.5 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-600" required>
                        <option value="">Pilih Kelas</option>
                        @foreach($kelas as $k)
                            <option value="{{ $k->id }}">{{ $k->nama_kelas }}</option>
                        @endforeach
