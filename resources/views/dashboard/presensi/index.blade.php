...
@section('content')
    ...
    <div class="flex flex-col lg:flex-row gap-6 items-start">
        ...
        <div class="w-full lg:w-1/3 bg-white rounded-xl shadow-xs border border-slate-200 p-5 lg:sticky lg:top-20 z-10"
            data-aos="fade-up" x-data="{ tab: 'kelas' }">
            <div class="flex items-center gap-3 mb-4 pb-3 border-b border-slate-100">
                <div
                    class="w-10 h-10 bg-blue-50 text-blue-700 rounded-lg flex items-center justify-center text-lg shrink-0 border border-blue-100">
            ...
            <form x-show="tab === 'kelas'" action="{{ route('dashboard.presensi.store') }}" method="POST" class="space-y-4">
                ...
                <div class="bg-blue-50/70 p-3 rounded-lg border border-blue-200/70 text-xs text-blue-800 leading-relaxed">
                    <i class="fas fa-info-circle text-blue-600 mr-1"></i> <strong>Sesi Kelas</strong>: Digunakan oleh Wali
                    Kelas di pagi hari. Siswa wajib <strong>scan wajah biometrik</strong> dari perangkat masing-masing.
                </div>

                <div>
                    <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">Pilih Kelas <span
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">Mata Pelajaran
                        <span class="text-[11px] font-normal text-slate-400 lowercase">(opsional)</span></label>
                    <div class="relative">
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="date" name="tanggal" value="{{ date('Y-m-d') }}" required
                            class="w-full pl-9 pr-3 py-2 bg-slate-50 border border-slate-300 rounded-lg text-xs font-medium text-slate-800 focus:ring-2 focus:ring-blue-600 focus:border-blue-600 transition">
            ...
            <!-- FORM SESI MAPEL -->
            <form x-cloak x-show="tab === 'mapel'" action="{{ route('dashboard.presensi.store') }}" method="POST"
                class="space-y-4">
                @csrf
                <input type="hidden" name="tipe" value="mapel">

                <div
                    class="bg-emerald-50/70 p-3 rounded-lg border border-emerald-200/70 text-xs text-emerald-900 leading-relaxed">
                    <i class="fas fa-circle-check text-emerald-600 mr-1"></i> <strong>Sesi Mapel</strong>: Data kehadiran
                    akan di-copy otomatis dari Sesi Kelas hari ini. Guru cukup mengedit siswa yang izin/sakit.
                </div>
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">Jam Pelajaran
                        (Les) <span class="text-rose-600">*</span></label>
                    <div class="relative">
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="date" name="tanggal" value="{{ date('Y-m-d') }}" required
                            class="w-full pl-9 pr-3 py-2 bg-slate-50 border border-slate-300 rounded-lg text-xs font-medium text-slate-800 focus:ring-2 focus:ring-emerald-600 focus:border-emerald-600 transition">
                        <i class="far fa-calendar absolute left-3 top-2.5 text-slate-400 text-xs"></i>
                    </div>
                </div>

                <button type="submit"
                    class="w-full bg-emerald-700 hover:bg-emerald-800 text-white font-bold py-2.5 rounded-lg text-xs transition flex items-center justify-center gap-2 shadow-xs">
        ...
        <div class="w-full lg:w-2/3 space-y-4">
            ...
            {{-- Filter Box --}}
            <div class="bg-white rounded-xl shadow-xs border border-slate-200 p-4 sm:p-5" data-aos="fade-up"
                data-aos-delay="50">
                {{-- Quick Filter Pills --}}
                ...
                <form method="GET" action="{{ route('dashboard.presensi') }}"
                    class="flex flex-col sm:flex-row items-end gap-3 flex-wrap">
                    <div class="w-full sm:w-auto flex-1 min-w-[140px]">
                        ...
                        <select name="kelas_id"
                            class="w-full px-3 py-2 bg-slate-50 border border-slate-300 rounded-lg text-xs font-medium">
                    ...
                    <div class="w-full sm:w-auto flex-1 min-w-[130px]">
                        ...
                        <input type="date" name="tanggal" value="{{ request('tanggal') }}" placeholder="Semua Tanggal"
                            class="w-full px-3 py-2 bg-slate-50 border border-slate-300 rounded-lg text-xs font-medium">
                    </div>
                    <div class="w-full sm:w-auto flex items-center gap-2">
                        <button type="submit"
                            class="w-full sm:w-auto bg-slate-900 text-white px-4 py-2 rounded-lg text-xs font-bold hover:bg-slate-800 transition flex items-center justify-center gap-1.5 shadow-xs">
                            <i class="fas fa-filter text-xs"></i> Filter
                        </button>
                        @if(request('kelas_id') || request('mapel_id') || request('tanggal') || request('bulan') || request('periode'))
                            <a href="{{ route('dashboard.presensi') }}"
                                class="w-full sm:w-auto text-xs font-semibold text-rose-600 hover:text-rose-800 hover:underline px-2 py-2 text-center whitespace-nowrap">
            ...
            <div class="bg-white rounded-xl shadow-xs border border-slate-200 overflow-hidden" data-aos="fade-up"
                data-aos-delay="80" x-data="{ openExport: false }">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-slate-100">
                        ...
                        <div class="p-5">
                            <div class="flex items-center gap-2.5 mb-3">
                                <div
                                    class="w-8 h-8 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-lg flex items-center justify-center">
                            ...
                            <form action="{{ route('dashboard.presensi.pdf.bulanan.kelas') }}" method="GET" target="_blank"
                                class="space-y-3">
                                <div>
                                ...
                                <div class="flex gap-2">
                                    <button type="submit" formaction="{{ route('dashboard.presensi.pdf.bulanan.kelas') }}"
                                        class="flex-1 bg-emerald-700 hover:bg-emerald-800 text-white text-xs font-bold py-2 rounded-lg transition flex items-center justify-center gap-1.5 shadow-xs">
                                        <i class="fas fa-file-pdf"></i> PDF
                                    </button>
                                    <button type="submit" formaction="{{ route('dashboard.presensi.excel.bulanan.kelas') }}"
                                        class="flex-1 bg-green-50 text-green-700 border border-green-200 hover:bg-green-100 text-xs font-bold py-2 rounded-lg transition flex items-center justify-center gap-1.5 shadow-xs">
                        ...
                        <div class="p-5">
                            <div class="flex items-center gap-2.5 mb-3">
                                <div
                                    class="w-8 h-8 bg-blue-50 text-blue-700 border border-blue-200 rounded-lg flex items-center justify-center">
                            ...
                            <form action="{{ route('dashboard.presensi.pdf.bulanan.mapel') }}" method="GET" target="_blank"
                                class="space-y-3">
                                <div>
                                    <label class="block text-[11px] font-bold text-slate-600 mb-1">Pilih Kelas</label>
                                    <select name="kelas_id" required
                                ...
                                <div class="flex gap-2">
                                    <button type="submit" formaction="{{ route('dashboard.presensi.pdf.bulanan.mapel') }}"
                                        class="flex-1 bg-blue-700 hover:bg-blue-800 text-white text-xs font-bold py-2 rounded-lg transition flex items-center justify-center gap-1.5 shadow-xs">
                                        <i class="fas fa-file-pdf"></i> PDF
                                    </button>
                                    <button type="submit" formaction="{{ route('dashboard.presensi.excel.bulanan.mapel') }}"
                                        class="flex-1 bg-green-50 text-green-700 border border-green-200 hover:bg-green-100 text-xs font-bold py-2 rounded-lg transition flex items-center justify-center gap-1.5 shadow-xs">
                                        <i class="fas fa-file-excel"></i> Excel
                                    </button>
                                </div>
            ...
            @forelse($sesiList as $index => $sesi)
                <div class="bg-white rounded-xl shadow-xs border border-slate-200 p-4 sm:p-5 flex flex-col md:flex-row gap-4 items-center justify-between transition-all duration-200 hover:border-blue-400 hover:shadow-md"
                    data-aos="fade-up" data-aos-delay="{{ 100 + ($index * 30) }}">

                    <div class="flex items-center gap-4 w-full md:w-auto">
                        {{-- Calendar Block Badge --}}
                        <div
                            class="w-14 h-14 rounded-lg {{ $sesi->tanggal->isToday() ? 'bg-blue-700 text-white shadow-xs' : 'bg-slate-100 border border-slate-200 text-slate-700' }} flex flex-col items-center justify-center shrink-0">
                            <span
                                class="text-[10px] font-bold uppercase tracking-wider">{{ $sesi->tanggal->isoFormat('MMM') }}</span>
                            <span
                                class="font-heading text-lg font-extrabold leading-none mt-0.5">{{ $sesi->tanggal->format('d') }}</span>
                        ...
                        <div class="min-w-0">
                            ...
                            <p class="text-xs text-slate-500 mt-1.5 flex items-center gap-2 flex-wrap">
                                ...
                                <span><i
                                        class="far fa-calendar text-slate-400 mr-1"></i>{{ $sesi->tanggal->isoFormat('dddd, D MMMM Y') }}</span>
                            </p>
                        </div>
                    </div>

                    <div
                        class="flex items-center justify-between md:justify-end gap-5 w-full md:w-auto mt-3 md:mt-0 pt-3 md:pt-0 border-t border-slate-100 md:border-none">
                        ...
                        <a href="{{ route('dashboard.presensi.detail', $sesi) }}"
                            class="btn-primary text-xs shrink-0 py-2 px-3.5">
            ...
            @empty
                <div class="bg-white rounded-xl border border-slate-200 border-dashed p-10 text-center" data-aos="fade-in"
                    data-aos-delay="100">
                    <div
                        class="w-14 h-14 bg-slate-50 border border-slate-200 rounded-xl flex items-center justify-center mx-auto mb-3.5 text-slate-400">
                        <i class="fas fa-calendar-xmark text-2xl"></i>
                    </div>
                    @if(request('kelas_id') || request('mapel_id') || request('tanggal') || request('bulan') || request('periode'))
                        <h3 class="font-heading text-slate-800 font-bold text-sm mb-1">Tidak ada sesi presensi yang sesuai filter
                        </h3>
                        <p class="text-xs text-slate-500 mb-4 max-w-md mx-auto">Tidak ditemukan pertemuan presensi untuk kriteria
                            yang dipilih. Coba ganti kelas, bulan, atau reset filter.</p>
                        <a href="{{ route('dashboard.presensi') }}"
                            class="btn-secondary text-xs inline-flex items-center gap-1.5 py-2 px-3.5">
                    ...
                    @else
                        ...
                        <p class="text-xs text-slate-500 max-w-sm mx-auto">Silakan pilih rombel kelas dan tanggal di panel sebelah
                            kiri untuk membuat sesi absensi baru.</p>
                    @endif
                </div>
            @endforelse
