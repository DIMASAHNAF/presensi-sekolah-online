<div align="center">
  ...
  # ⚡ SMART AI FACE RECOGNITION, GEOFENCING & WIFI IP WHITELIST
  ### Sistem Presensi Sekolah Modern Berbasis AI Biometric Face ID, GPS Geofencing Radius, MikroTik WiFi Whitelist, & Realtime Live Sync
  ...
  <p align="center" style="margin-top: 12px; margin-bottom: 16px;">
    ...
    <a href="#"><img src="https://img.shields.io/badge/WiFi_Security-IP_Whitelist_🔒-0284C7?style=for-the-badge&logo=wi-fi&logoColor=white" alt="WiFi IP Whitelist"></a>
  ...
  <p align="center">
    <strong>Aplikasi presensi mandiri siswa tercanggih dengan verifikasi wajah AI (128-D biometric vector), validasi radius GPS sekolah (Haversine Formula), proteksi jaringan WiFi resmi sekolah (MikroTik Subnet & IP Public Whitelist), sinkronisasi realtime tanpa reload, pembagian sesi harian & 40+ mata pelajaran, serta generator laporan PDF & Excel resmi.</strong>
  </p>
...
## 🚀 Sekilas Sistem Baru: Proteksi Keamanan 3-Lapis (Triple Security Layer)

Sistem ini dirancang khusus untuk mengatasi segala bentuk kecurangan presensi digital (titip absen, manipulasi Fake GPS, screenshot barcode, hingga absen dari luar sekolah menggunakan paket data seluler).

```
   TRADISIONAL (OLD)                               SISTEM CERDAS (NOW)
┌─────────────────────────┐            ┌──────────────────────────────────────────────┐
│ 📷 Scan QR Code          │            │ 🤖 AI Face ID Scanner (128-D Vector ResNet)  │
│ (Rentan titip absen,     │   ====>    │ 📍 GPS Geofencing Lock (Haversine Formula)   │
│ foto QR dioper, & bisa  │            │ 📶 School WiFi IP & MikroTik CIDR Whitelist  │
│ di-scan dari rumah)     │            │ ⚡ Live Auto-Sync Tanpa Refresh Browser      │
└─────────────────────────┘            └──────────────────────────────────────────────┘
```

> [!IMPORTANT]
> **Tingkat Keamanan Berlapis (Triple Security Layer)**: Siswa hanya dapat tercatat **HADIR** jika:
> 1. 👤 **Wajah Cocok**: Verifikasi biometrik kecocokan wajah AI (*Python dlib ResNet Euclidean Distance ≤ 0.45*).
> 2. 📍 **Lokasi Sah**: Berada di dalam radius batas sekolah (*Haversine Distance Formula*).
> 3. 📶 **Jaringan Resmi**: Terhubung langsung ke jaringan WiFi resmi / Hotspot MikroTik sekolah (*IP Public WAN & Subnet CIDR Whitelist*).

---

## ✨ Fitur Unggulan (Next-Gen Biometrics & Network Security)

<!-- ANIMATED SVG CARDS: DUAL SECURITY & LIVE SYNC -->
<p align="center">
  <img src="public/images/readme/dual-security-hud.svg" alt="Triple Security & Live Sync Features" width="100%" />
</p>
...
### 📶 3. School WiFi IP & MikroTik Hotspot Whitelist (Anti-Bocor 100%)
* 🔒 **Kunci Jaringan WiFi Sekolah**: Mencegah siswa absen menggunakan paket data seluler pribadi (Telkomsel, Indosat, XL) atau WiFi warung sekitar. Siswa **wajib terhubung ke WiFi resmi sekolah**.
* 🖧 **Dukungan Subnet CIDR MikroTik**: Mendukung format subnet CIDR (contoh: `10.10.10.254/21`), otomatis mencakup ribuan IP DHCP Hotspot siswa (`10.10.8.1 - 10.10.15.254`) tanpa perlu input manual satu per satu.
* ☁️ **Cloudflare & Reverse Proxy Awareness**: Mendeteksi IP publik asli klien secara akurat melalui header proxy (`CF-Connecting-IP`, `X-Forwarded-For`, `X-Real-IP`).
* 🔘 **Tombol 1-Click "+ Tambahkan IP Ini"**: Jika router ISP (Indihome/Biznet) restart dan mendapatkan IP dinamis baru, Admin cukup buka menu Pengaturan lewat WiFi sekolah dan klik tombol *"+ Tambahkan IP Ini"* tanpa perlu koding ulang.
* 👁️ **Graceful Dashboard Experience**: Siswa di luar WiFi sekolah tetap bisa membuka website untuk mengecek jadwal atau riwayat kehadiran, namun tombol pemindaian wajah otomatis **terkunci rapat**.

### ⚡ 4. Realtime Live Auto-Sync (Tanpa Reload Halaman)
...
### 📚 5. Master 40+ Mata Pelajaran & Sesi Berjenjang
...
### 🛡️ 6. Manajemen Wajah & Audit Trail Admin
...
### 📄 7. Generator Rekap PDF & Excel Cerdas
| Jenis Laporan | Deskripsi & Format Cetak |
|---|---|
| 👨‍🏫 **Rekap Bulanan Wali Kelas** | Menampilkan seluruh tanggal aktif sesi sekolah, akumulasi rekapitulasi `H`, `S`, `I`, `A`, persentase kehadiran riil, dan status evaluasi kedisiplinan siswa (PDF & Excel). |
...
sequenceDiagram
    ...
    Siswa->>Laravel: Buka Dashboard Siswa (Polling Sesi + Validasi Jaringan)
    Laravel-->>Siswa: Sesi Ditemukan + Info Geofencing & Status IP WiFi
    alt IP Tidak Terdaftar (Bukan WiFi Sekolah)
        Siswa->>Siswa: Tombol Kamera Terkunci ❌ ("Harus Pakai WiFi Sekolah")
    else Terhubung ke WiFi Resmi Sekolah
        Siswa->>Siswa: Tombol Kamera Aktif ✅ (Face-API HUD Oval & GPS Coordinates)
        Siswa->>Laravel: Submit Foto Wajah (Base64) + Lat/Long GPS + Client IP

        Note over Laravel,Python: 4. VALIDASI RADIUS, IP & BIOMETRIK
        Laravel->>Laravel: Validasi IP Whitelist (WAN / Subnet CIDR)
        Laravel->>Laravel: Hitung Jarak GPS via Haversine Formula
        alt Jarak > Radius Sekolah (di luar batas)
...
| **Network Security** | CIDR Subnet Parser & Reverse Proxy | Validasi IP Public & Subnet Hotspot MikroTik (`/21`) |
| **User Interface** | ![Tailwind](https://img.shields.io/badge/Tailwind_CSS-3.x-38B2AC?style=flat-square&logo=tailwind-css&logoColor=white) | Desain antarmuka modern, clean solid accents, responsif |
| **Frontend Reactive** | ![Alpine.js](https://img.shields.io/badge/Alpine.js-3.x-8BC0D0?style=flat-square&logo=alpine.js&logoColor=white) | Modal scanner, polling interval 2 detik, reactivity ringan |
| **Database** | ![MySQL](https://img.shields.io/badge/MySQL-8.0+-4479A1?style=flat-square&logo=mysql&logoColor=white) | Relasional storage, index query, json casting face descriptor |
...
|---|---|---|---|
| 👑 **Admin** | `admin` | `password` | Kelola master Siswa, Guru, Kelas, Log Presensi, Reset Wajah, Pengaturan Radius GPS & Whitelist WiFi Sekolah |
| 👨‍🏫 **Guru / Wali Kelas** | `198501012010011001` (NIK Guru) | `password` | Buka sesi kelas & mapel, monitor live sync, koreksi manual status presensi, cetak rekap PDF & Excel |
| 🧑‍🎓 **Siswa** | `0081234567` (NISN Siswa) | `password` | Akses dashboard siswa, enroll wajah mandiri, scan presensi Face ID + GPS + WiFi |
...
## 📍 Pengaturan Keamanan: Geofencing & Whitelist WiFi Sekolah

1. Masuk ke panel **Admin** -> Menu **Pengaturan Lokasi** (`/dashboard/pengaturan/lokasi`).
2. **GPS Geofencing**:
   - Geser pin pada **Peta Interaktif Leaflet** ke posisi gedung sekolah Anda, atau masukkan koordinat Latitude dan Longitude secara manual.
   - Tentukan **Radius Toleransi (Meter)** (contoh: `100` meter).
   - Aktifkan toggle **Aktifkan Geofencing**.
3. **Whitelist IP Jaringan WiFi Sekolah**:
   - Aktifkan toggle **Aktifkan Pembatasan Jaringan WiFi**.
   - Masukkan Subnet IP MikroTik Hotspot (contoh: `10.10.10.254/21`) dan IP Public WAN sekolah Anda pada kolom teks.
   - Atau cukup klik tombol **"+ Tambahkan IP Ini"** saat admin terhubung ke WiFi sekolah untuk mendeteksi IP koneksi secara otomatis.
4. Klik **Simpan Pengaturan**.

---

## 💡 Konfigurasi MikroTik Hotspot (Opsional - Walled Garden)

Jika sekolah Anda menggunakan sistem **MikroTik Hotspot Login**, tambahkan aturan **Walled Garden** agar siswa dapat langsung presensi tanpa harus membeli voucher internet:

1. Di Winbox MikroTik, buka menu **IP** > **Hotspot** > tab **Walled Garden**.
2. Klik **`+` (Add)**:
   - **Action**: `allow`
   - **Dst. Host**: `presensi.tiksmkn1beringin.my.id`
3. *(Opsional)* Di tab **Walled Garden IP List**, klik **`+` (Add)**:
   - **Action**: `accept`
   - **Dst. Address**: `10.10.10.223` (IP lokal server presensi di jaringan sekolah)

---

## 📄 Format Laporan Presensi PDF & Excel
...
* 📊 **Ekspor Excel Rapi**: Rekapitulasi kehadiran lengkap per kelas atau mapel dapat diekspor langsung ke format `.xlsx`.
